...
internal sealed class CPUTextureARGB4444(CPUTextureData data, int width, int height, int mipCount)
    : CPUTexture2D(data, width, height, mipCount)
{
    public override TextureFormat format => TextureFormat.ARGB4444;
    ...
    public override Color GetPixel(int x, int y, int mipLevel = 0)
    {
        ...
        float a = ((hi >> 4) & 0xF) * (1f / 15f);
        float r = (hi & 0xF) * (1f / 15f);
        float g = ((lo >> 4) & 0xF) * (1f / 15f);
        float b = (lo & 0xF) * (1f / 15f);
