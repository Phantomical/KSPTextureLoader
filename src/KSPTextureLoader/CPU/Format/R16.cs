...
partial class CPUTexture2D
{
    ...
    public readonly struct R16 : ICPUTexture2D, IGetPixels
    {
        public int Width { get; }
        public int Height { get; }
        public int MipCount { get; }
        public TextureFormat Format => TextureFormat.R16;

        readonly LargeNativeArray<ushort> data;

        internal R16(LargeNativeArray<byte> data, int width, int height, int mipCount)
        {
        ...
        public R16(NativeArray<byte> data, int width, int height, int mipCount)
        ...
        {
            return GetPixels<R16, ushort, GetPixelsJob>(
                in this,
                mipLevel,
                allocator,
        ...
        {
            return GetPixels32<R16, ushort, GetPixels32Job>(
                in this,
                mipLevel,
                allocator,
        ...
        struct GetPixelsJob : IJobParallelForBatch
        {
            ...
            public void Execute(int start, int count)
            {
                ...
                for (int i = start; i < end; ++i)
                    pixels[i] = new Color(data[i] * UShort2Float, 1f, 1f, 1f);
            }
        }
        ...
        struct GetPixels32Job : IJobParallelForBatch
        {
            ...
            public void Execute(int start, int count)
            {
                int end = start + count;
                for (int i = start; i < end; ++i)
                    pixels[i] = new Color32((byte)(data[i] >> 8), 255, 255, 255);
            }
        }
    }
}
