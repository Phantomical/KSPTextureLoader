...
public static class OnDemandStorage
{
    ...
    public static Byte[] LoadWholeFile(String path)
    {
        ...
        using (FileStream file = File.OpenRead(path))
        {
            ...
            while (fileBytes > 0)
            {
                Int32 read = file.Read(
                    _wholeFileBuffer,
                    i,
                    fileBytes > 0x100000 ? 0x100000 : fileBytes
                );
                if (read <= 0)
                {
                    continue;
    ...
    private static unsafe void CalculateArrayLengthOffset()
    {
        ...
        fixed (Int32* ptr = &temp[0])
        {
            ...
            Debug.Log(
                "[Kopernicus] CalculateArrayLengthOffset using offset of " + _arrayLengthOffset
            );
        }
    }
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(File.OpenRead(path)))
                    {
                        ...
                        if (num == DDSValues.uintMagic)
                        {
                            ...
                            if (ddsHeader.ddspf.dwFourCC == DDSValues.uintDX10)
                            {
                                ...
                                Debug.Log(
                                    $"parsed dds10 header for {path} -  {ddsHeader10.dxgiFormat}"
                                );
                            }
                            ...
                            if (fourcc)
                            {
                                ...
                                {
                                    map = new Texture2D(
                                        (Int32)ddsHeader.dwWidth,
                                        (Int32)ddsHeader.dwHeight,
                                        TextureFormat.DXT1,
                                        mipmap
                                    );
                                ...
                                else if (ddsHeader.ddspf.dwFourCC == DDSValues.uintDX10)
                                {
                                    switch (ddsHeader10.dxgiFormat)
                                    {
                                        ...
                                        case DXGI_FORMAT.DXGI_FORMAT_BC5_UNORM:
                                            Debug.Log(
                                                $"[Kopernicus]: Loading {path} with format {ddsHeader10.dxgiFormat}"
                                            );
                                            map = new Texture2D(
                                                (int)ddsHeader.dwWidth,
                                                (int)ddsHeader.dwHeight,
                                                TextureFormat.BC5,
                                                mipmap,
                                                linear: false
                                            );
                                            map.LoadRawTextureData(LoadRestOfReader(binaryReader));
                                            Debug.Log(
                                                $"[Kopernicus]: Loaded {path} with format {ddsHeader10.dxgiFormat}"
                                            );
                                            break;
                                        ...
                                        default:
                                            Debug.LogError(
                                                $"[Kopernicus]: DDS format {ddsHeader10.dxgiFormat} not supported: "
                                                    + path
                                            );
                                            break;
                                    }
                            ...
                            if (!fourcc)
                            {
                                ...
                                )
                                {
                                    ...
                                    ddsHeader.ddspf.dwRGBBitCount == 4
                                    || ddsHeader.ddspf.dwRGBBitCount == 8
                                )
                                {
                                    try
                                    {
                                        ...
                                        if (length - 128 >= pixels)
                                        {
                                            ...
                                            {
                                                image[(i - 4 * colors) * 8 / bpp] = palette[
                                                    data[i] * colors / 256
                                                ];
                                                if (bpp == 4)
                                                {
